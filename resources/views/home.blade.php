...
<<<<<<< HEAD
                <p class="text-blue-500 mt-4">Posted by:<a href="{{ route('users.show', $post->user->id) }}" class="text-blue-500 hover:underline">
                {{ $post->user->name }}
...
=======
                {{-- <p class="text-gray-500 mt-4">Posted by: {{ $post->user->name:'Unknown' }}</p> --}}
                <p class="text-gray-500 mt-4">Posted by: {{ $post->user ? $post->user->name : 'Unknown' }}</p>
>>>>>>> 5a27bb9 (fix bug)
            </div>
        @endforeach
    </div>
